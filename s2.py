...
#        super().__init__(*args, directory=server_base_path, **kwargs)
#        
    def do_OPTIONS(self):
...
#        time.sleep(1)
#        try:
#            game_list=os.listdir(game_base_path)
#            s='<!DOCTYPE html><html><head><link rel="icon" href="https://serverpg.herokuapp.com/favicon.ico"><title>Game List</title></head>'
#            for e in game_list:
#                s+=datetime.datetime.fromtimestamp(os.path.getctime(e+'.zip')).strftime('%Y/%m/%d-%H:%M:%S')+' '+str(int(os.path.getsize(e+'.zip')/10485.76)/100)+'MB<br><a href="'+game_base_path+e+'" target="_blank" rel="noopener noreferrer">'+e+'</a><br>'
